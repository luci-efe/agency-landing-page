...
describe('ServiceShowcase Component', () => {
  ...
  it('displays service descriptions', () => {
    ...
    expect(screen.getByText('Modern websites and high-performance web applications')).toBeInTheDocument();
    expect(screen.getByText('Native and cross-platform applications')).toBeInTheDocument();
    expect(screen.getByText('Strategy and digital transformation')).toBeInTheDocument();
  });

  it('includes interactive elements indicator', () => {
    renderWithIntl(<ServiceShowcase />);
    
    expect(screen.getByText('Ready for Interactive Development')).toBeInTheDocument();
  });
